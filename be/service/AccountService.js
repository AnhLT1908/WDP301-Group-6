...
export const getManagerAccounts = async (req, res) => {
  try {
    ...
    const totalManagers = await Account.countDocuments({
      accountType: "Manager",
    });
...
export const getProfile = async (req, res) => {
  try {
    ...
    const { password, refreshToken, passwordResetCode, imageStores, ...other } =
      profile._doc;
    return res.status(200).json({
      data: other,
    });
...
export const setManagerInactive = async (req, res, next) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(managerId)) {
      return res
        .status(400)
        .json({ success: false, message: "managerId không hợp lệ!" });
    }
    ...
    if (!manager || manager.accountType !== "Manager") {
      return res
        .status(404)
        .json({ success: false, message: "Không tìm thấy Manager!" });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Chỉ Admin hoặc Manager mới có quyền thay đổi trạng thái isContact!",
      });
    }
    ...
    await Notification.create({
      ...
      message: `Trạng thái người đại diện của bạn đã được cập nhật thành ${
        isContact ? "bật" : "tắt"
      }.`,
    });
    ...
    return res.status(200).json({
      success: true,
      message: `Trạng thái isContact đã được cập nhật thành ${
  ...
  try {
    ...
    if (manager.status === false) {
      return res.status(400).json({
        success: false,
        message:
          "Không thể chuyển Manager vì trạng thái hiện tại là không hoạt động (status: false)!",
      });
    }
    // Kiểm tra House
